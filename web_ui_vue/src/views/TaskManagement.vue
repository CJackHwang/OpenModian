  ...
  <div>
    ...
    <div class="app-section">
      <div class="d-flex align-center">
        ...
        <div>
          <h1 class="text-h4 font-weight-medium mb-1">任务管理</h1>
        ...
        <v-card color="primary-container" class="app-card stats-card">
          <v-card-text class="p-lg">
            <div class="d-flex align-center">
              ...
              <div>
                <div
                  class="text-h4 font-weight-medium text-on-primary-container"
                >
                  {{ normalTasksCount }}
                </div>
                <div class="text-subtitle-2 text-on-primary-container-variant">
                  运行中任务
                </div>
              </div>
            </div>
        ...
        <v-card color="success-container" class="app-card stats-card">
          <v-card-text class="p-lg">
            <div class="d-flex align-center">
              ...
              <div>
                <div
                  class="text-h4 font-weight-medium text-on-success-container"
                >
                  {{ activeScheduledTasksCount }}
                </div>
                <div class="text-subtitle-2 text-on-success-container-variant">
                  活跃定时任务
                </div>
              </div>
            </div>
        ...
        <v-card color="warning-container" class="app-card stats-card">
          <v-card-text class="p-lg">
            <div class="d-flex align-center">
              ...
              <div>
                <div
                  class="text-h4 font-weight-medium text-on-warning-container"
                >
                  {{ pausedScheduledTasksCount }}
                </div>
                <div class="text-subtitle-2 text-on-warning-container-variant">
                  暂停定时任务
                </div>
              </div>
            </div>
          ...
          <v-card-text class="p-lg">
            <div class="d-flex align-center">
              <v-avatar color="info" size="48" class="me-3">
                <v-icon color="on-info" size="24"
                  >mdi-format-list-bulleted</v-icon
                >
              </v-avatar>
              <div>
                <div class="text-h4 font-weight-medium text-on-info-container">
                  {{ totalTasksCount }}
                </div>
                <div class="text-subtitle-2 text-on-info-container-variant">
                  总任务数
                </div>
              </div>
            </div>
    ...
    <v-card class="app-card app-table">
      ...
      >
        ...
        <template #item.status="{ item }">
          <div class="d-flex align-center">
            <div :class="['app-status', `status-${item.stats.status}`]"></div>
            <v-chip
        ...
        <template #item.progress="{ item }">
          <div v-if="item.task_type === 'normal'">
            <v-progress-linear
              ...
              rounded
            />
            <div class="text-caption mt-1">{{ item.stats.progress || 0 }}%</div>
          </div>
          <div v-else class="text-caption">
            ...
            <div class="mt-1">
              <strong>间隔:</strong>
              {{ formatInterval(item.schedule_info.interval_seconds) }}
            </div>
          </div>
        </template>
    ...
    <v-dialog v-model="detailDialog" max-width="800px">
      <v-card v-if="selectedTask">
        <v-card-title class="d-flex align-center">
          <v-icon class="me-2">{{
            selectedTask.task_type === "scheduled"
              ? "mdi-clock-outline"
              : "mdi-play-circle"
          }}</v-icon>
          任务详情
              ...
              <v-list density="compact">
                ...
                <v-list-item>
                  ...
                  <v-list-item-subtitle>
                    <v-chip
                      size="small"
                      :color="getStatusColor(selectedTask.stats.status)"
                    >
            ...
            <v-col cols="12" md="6">
              <v-list density="compact">
                <v-list-item>
                  ...
                  <v-list-item-subtitle
                    >{{ selectedTask.config.start_page }}-{{
                      selectedTask.config.end_page
                    }}</v-list-item-subtitle
                  >
                </v-list-item>
                <v-list-item>
              ...
              <v-col cols="12" md="6">
                <v-list density="compact">
                  <v-list-item v-if="selectedTask.schedule_info.next_run_time">
                    ...
                    <v-list-item-subtitle>{{
                      formatDateTime(selectedTask.schedule_info.next_run_time)
                    }}</v-list-item-subtitle>
                  </v-list-item>
                  <v-list-item v-if="selectedTask.schedule_info.last_run_time">
                    <v-list-item-title>上次执行时间</v-list-item-title>
                    <v-list-item-subtitle>{{
                      formatDateTime(selectedTask.schedule_info.last_run_time)
                    }}</v-list-item-subtitle>
                  </v-list-item>
                </v-list>
              </v-col>
              <v-col cols="12" md="6">
                <v-list density="compact">
                  <v-list-item>
                    ...
                    <v-list-item-subtitle>{{
                      selectedTask.schedule_info.run_count
                    }}</v-list-item-subtitle>
                  </v-list-item>
                  <v-list-item>
                    ...
                    <v-list-item-subtitle>
                      <v-chip
                        size="small"
                        :color="selectedTask.is_active ? 'success' : 'warning'"
                      >
...
const activeScheduledTasksCount = computed(() => {
  return tasks.value.filter(
    (task) => task.task_type === "scheduled" && task.is_active,
  ).length;
});
...
const toggleScheduledTask = async (task) => {
  try {
    const response = await axios.post(
      `/api/scheduled_tasks/${task.task_id}/toggle`,
    );
    if (response.data.success) {
      // 重新加载任务列表
      await loadTasks();
...
const runScheduledTaskNow = async (task) => {
  ...
  try {
    // 这里需要添加立即执行定时任务的API
    const response = await axios.post(
      `/api/scheduled_tasks/${task.task_id}/run_now`,
    );
    if (response.data.success) {
      alert("任务已开始执行");
      await loadTasks();
