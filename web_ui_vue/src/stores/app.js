...
export const useAppStore = defineStore("app", () => {
  ...
  const loadTasks = async () => {
    try {
      ...
      if (response.data && response.data.success) {
        ...
        // 统计任务状态
        systemStats.activeTasks = tasks.filter(
          (t) =>
            t.stats &&
            (t.stats.status === "running" || t.stats.status === "starting"),
        ).length;

        systemStats.completedTasks = tasks.filter(
          (t) => t.stats && t.stats.status === "completed",
        ).length;

        systemStats.failedTasks = tasks.filter(
          (t) =>
            t.stats &&
            (t.stats.status === "failed" || t.stats.status === "error"),
        ).length;
        ...
        if (runningTask) {
          ...
          currentTask.id &&
          !["completed", "failed", "stopped"].includes(currentTask.status)
        ) {
  ...
  const refreshData = async () => {
    await Promise.all([loadSystemStats(), loadTasks()]);
  };
