 ...
 * @returns {Object} 验证结果
 */
function validateContrast(foreground, background, level = 'AA', size = 'normal') {
  ...
  const report = {
    ...
    summary: {
      light: {
        ...
        passed: Object.values(lightResults).filter(r => r.normal.passes).length,
        failed: Object.values(lightResults).filter(r => !r.normal.passes).length
      },
      dark: {
        total: Object.keys(darkResults).length,
        passed: Object.values(darkResults).filter(r => r.normal.passes).length,
        failed: Object.values(darkResults).filter(r => !r.normal.passes).length
      }
    }
  };
