  ...
  <v-card class="mb-4 app-card">
    <v-card-title class="p-lg">
      ...
      <v-sheet class="flex-grow-1" color="transparent">
        <v-card-text class="text-h6 font-weight-bold pa-0">高级筛选构建器</v-card-text>
    ...
    <v-card-text class="p-lg pt-0">
      <!-- 筛选条件列表 -->
      <v-sheet v-for="(filter, index) in filters" :key="index" class="mb-3" color="transparent">
        <v-row align="center">
          <v-col cols="12" md="3">
            <v-select
...
// 响应式数据
const filters = ref([{ field: "", operator: "", value: "", logic: "AND" }]);

const sortConfig = ref([{ field: "", order: "desc", priority: 1 }]);
...
const applyFilters = () => {
  const activeFilters = filters.value.filter(
    (f) => f.field && f.operator && f.value !== "",
  );
  const activeSort = sortConfig.value
    .filter((s) => s.field)
...
const generateSQL = () => {
  ...
  if (activeSort.length > 0) {
    const orderConditions = activeSort.map(
...
const loadSavedConfigs = () => {
  savedConfigs.value = JSON.parse(
    localStorage.getItem("filterConfigs") || "[]",
  );
};
