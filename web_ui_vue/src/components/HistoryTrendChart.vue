  ...
  <v-card class="app-card">
    ...
    <v-card-text class="p-lg pt-0">
      ...
      <div v-if="!historyData.length" class="text-center py-8">
        <v-icon size="64" color="on-surface-variant">mdi-chart-line-variant</v-icon>
        <div class="text-h6 mt-2 text-on-surface">暂无历史数据</div>
        <div class="text-caption text-on-surface-variant">需要至少2条历史记录才能显示趋势图</div>
      </div>
    </v-card-text>
  </v-card>
</template>
...
const chartData = computed(() => {
  ...
  const datasets = selectedMetrics.value.map((metricKey, index) => {
    ...
    // 为每个数据集分配不同颜色
    const colors = ['#1976d2', '#4caf50', '#ff9800', '#f44336', '#2196f3', '#9c27b0']
...
const updateChart = () => {
  if (!chart.value || !chartData.value) {
    console.log('无法更新图表:', { hasChart: !!chart.value, hasData: !!chartData.value })
    return
  }
...
// 监听器
watch([selectedMetrics, timeRange], () => {
